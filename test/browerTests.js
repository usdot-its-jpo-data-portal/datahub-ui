...
beforeEach(function () {
    ...
    else if (browserIdx === 1) {
        browser = new webdriver.Builder().usingServer().withCapabilities({ 'browserName': 'firefox' }).build()
        browser.manage().window().maximize();
    }
    else if (browserIdx === 2) {
        browser = new webdriver.Builder().usingServer().withCapabilities({ 'browserName': 'ie' }).build()
        browser.manage().window().maximize();
    }
    //return browser.get("https://www.its.dot.gov/data/");
